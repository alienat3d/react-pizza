...
const Header: React.FC = () => {
    ...
    // 26.0 Итак, нам нужно также сохранять содержимое корзины в localStorage, чтобы у пользователя оно сохранялось, даже при закрытии браузера или обновлении страницы.
    // 26.1.0 Здесь нам пригодится хук "useEffect", в котором укажем зависимостью стейт корзины. Далее нам понадобится метод объекта JSON "stringify", который будет преобразовывать массив, добавленных пицц в корзину в строчку (ведь в localStorage всё записывается строчным типом данных). После чего, с помощью метода setItem мы запишем содержимое корзины в localStorage.
    // 26.1.1 Но нам нужна также проверка, что если это первый рендер, то не перезаписывать localStorage.
    // (Go to [cartSlice.ts])
    React.useEffect(() => {
